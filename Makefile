...
MESSAGES_NPERIOD_H=$(STATICINCLUDE)/messages_nperiod.h

GEN_HEADERS = $(MESSAGES_H) $(UBX_PROTOCOL_H) $(MTK_PROTOCOL_H) $(XSENS_PROTOCOL_H) $(DL_PROTOCOL_H) $(ABI_MESSAGES_H) $(MESSAGES_NPERIOD_H)
...
$(MESSAGES_NPERIOD_H) : $(MESSAGES_XML) generators
	$(Q)test -d $(STATICINCLUDE) || mkdir -p $(STATICINCLUDE)
	@echo GENERATE $@
	$(eval $@_TMP := $(shell $(MKTEMP)))
	$(Q)PAPARAZZI_SRC=$(PAPARAZZI_SRC) PAPARAZZI_HOME=$(PAPARAZZI_HOME) $(GENERATORS)/gen_messages_nperiod.out $< telemetry > $($@_TMP)
	$(Q)mv $($@_TMP) $@
	$(Q)chmod a+r $@
