...
void autopilot_init(void)
{
  ...
  register_periodic_telemetry(DefaultPeriodic, MESG_ROTORCRAFT_STATUS, send_status);
  register_periodic_telemetry(DefaultPeriodic, MESG_ENERGY, send_energy);
  register_periodic_telemetry(DefaultPeriodic, MESG_ROTORCRAFT_FP, send_fp);
  register_periodic_telemetry(DefaultPeriodic, MESG_ROTORCRAFT_CMD, send_rotorcraft_cmd);
  register_periodic_telemetry(DefaultPeriodic, MESG_DL_VALUE, send_dl_value);
#ifdef ACTUATORS
  register_periodic_telemetry(DefaultPeriodic, MESG_ACTUATORS, send_actuators);
#endif
#ifdef RADIO_CONTROL
  register_periodic_telemetry(DefaultPeriodic, MESG_RC, send_rc);
  register_periodic_telemetry(DefaultPeriodic, MESG_ROTORCRAFT_RADIO_CONTROL, send_rotorcraft_rc);
#endif
}
